...
type CreateReviewTypes = {
  handleCreateReview: SubmitHandler<FormTypes>;
  loading: boolean;
};

export const CreateReview: React.FC<CreateReviewTypes> = ({
  handleCreateReview,
  loading,
}: CreateReviewTypes) => {
  const {
  ...
  const clearInput = (
    name: 'price' | 'title' | 'description' | 'dayOfPurchase'
  ) => {
    resetField(name);
  };
  ...
  return (
    <Form onSubmit={handleSubmit(handleCreateReview)}>
      ...
      <Button color="purple" size="all" type="submit">
